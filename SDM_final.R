...
#remove _ and spaces from names so they merge properly
xy <- merged_locs %>%
...
#join locations with raw detections
data <- merged_dets %>%
  left_join(xy)



#### Spatial thinning dataset to remove collinearity ####
...
#after thinning 282 spatially independent sites remaining
...
##Asian elephant## not used in thesis as too few detections meant unreliable predictions
...
#univariate models to find the most representative scale for each variable for each species

#read presence/absence spatial points data frame (spdf) data
...
#repeat each time for new species, reading in the relevant shape file in the first step. 
#most representative scale was selected based on the AUC score, and recorded in an excel document
#a set of 15 scale-optimised covariates were placed in new folders for each species, ready for the SDM modelling
...
#VIF to remove multicollinearity. We were happy with variables once they were all under 0.5 for VIF score
vif(env)
...
#plot variable importance for each species. plot shows mean of best performing models, with confidence intervals. 
